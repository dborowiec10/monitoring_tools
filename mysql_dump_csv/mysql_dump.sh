...
DEFINE_string 'host' '0.0.0.0' 'mysql hostname' 'H'
DEFINE_string 'user' 'root' 'mysql user' 'u'
DEFINE_integer 'port' 3306 'mysql port' 'P'
DEFINE_string 'password' 'password' 'mysql password' 'p'
DEFINE_string 'query' 'SELECT * FROM power.measurement;' 'mysql query' 'q'
DEFINE_string 'out' 'vgg19_bs96.csv' 'out file path' 'o'
DEFINE_string 'start' '2020-01-20 13:56:50' 'start of query range' 's'
DEFINE_string 'end' '2020-01-20 14:01:39' 'end of query range' 'e'
...
PORT=3306
PASSWORD='password'
QUERY='SELECT * FROM power.measurement;'
...
for i in "${!STARTS[@]}"; do
  mysql -h ${HOST} -u ${USER} -P ${PORT} -p${PASSWORD} -e "SELECT * FROM power.measurement WHERE time >= '${STARTS[$i]}.000' AND time <= '${ENDS[$i]}.999';" | sed 's/\t/,/g' > ${JOBS[$i]}.csv
done

for filename in ./*.csv; do
    mkdir -p data/${filename%.csv}/tektronix_power_watts
    mv $filename data/${filename%.csv}/tektronix_power_watts/$filename
done